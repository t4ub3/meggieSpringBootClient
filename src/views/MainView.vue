  ...
  methods: {
    parseRecords (recordData) {
      ...
      for (let record of recordData.drivingRecords) {
        records.push({type: 'drivingRecord', date: record.date, mileage: record.mileage})
      }
      for (let record of recordData.fuelRecords) {
        records.push({type: 'fuelRecord', date: record.date, mileage: record.mileage})
      }
      for (let record of recordData.paymentRecords) {
        records.push({type: 'paymentRecord', date: record.date, mileage: record.mileage})
      }
      return records
    },
    getIconName (type) {
